...
class SpecialEnv : public EnvWrapper {
 public:
  ...
  Status NewRandomAccessFileWithSize(const std::string& f, RandomAccessFile** r, size_t start, size_t size) {
    class CountingFile : public RandomAccessFile {
     private:
      RandomAccessFile* target_;
      AtomicCounter* counter_;
    ...
    Status s = target()->NewRandomAccessFileWithSize(f, r, start,size);
    if (s.ok() && count_random_reads_) {
      *r = new CountingFile(*r, &random_read_counter_);
    }
    return s;
...
TEST(DBTest, CompactionsGenerateMultipleFiles) {
    fprintf(stderr, "THIS IMPLEMENTATION CAN GENERATE ONE FILE WHILE COMPACTION\n");
  Options options = CurrentOptions();
...
void BM_LogAndApply(int iters, int num_base_files) {
  ...
  for (int i = 0; i < num_base_files; i++) {
    ...
    vbase.AddFile(2, fnum++, 1 /* file size */, start, limit, 0);
  }
  ...
  for (int i = 0; i < iters; i++) {
    VersionEdit vedit;
    f->number=fnum;
    vedit.DeleteFile(2, f);//num);
    InternalKey start(MakeKey(2*fnum), 1, kTypeValue);
    InternalKey limit(MakeKey(2*fnum+1), 1, kTypeDeletion);
    vedit.AddFile(2, fnum++, 1 /* file size */, start, limit, 0); 
