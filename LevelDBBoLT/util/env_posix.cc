...
class PosixMmapReadableFileWithSize final : public RandomAccessFile {
 public:
  PosixMmapReadableFileWithSize(std::string filename, char* mmap_base, size_t length, size_t start,
                        Limiter* mmap_limiter)
      : mmap_base_(mmap_base), length_(length), mmap_limiter_(mmap_limiter), start_(start),
        filename_(std::move(filename)) {}

  ~PosixMmapReadableFileWithSize() override {
...
class PosixEnv : public Env {
 public:
  ...
  Status NewRandomAccessFileWithSize(const std::string& filename,
                             RandomAccessFile** result, uint64_t start,uint64_t endOffset) override {
    *result = nullptr;
    int fd = ::open(filename.c_str(), O_RDONLY);
    if (fd < 0) {
      return PosixError(filename, errno);
    ...
    if (true || !mmap_limiter_.Acquire()) {
      *result = new PosixRandomAccessFile(filename, fd, &fd_limiter_);
      return Status::OK();
    }

    uint64_t file_size = endOffset;
    ...
    if (status.ok()) {
      ...
      if (mmap_base != MAP_FAILED) {
        ...
        *result = new PosixMmapReadableFile(
            ...
            &mmap_limiter_);
            */
        *result = new PosixMmapReadableFileWithSize(
            filename, reinterpret_cast<char*>(mmap_base), file_size, start,
            &mmap_limiter_);
      } else {
