...
namespace leveldb {
    ...
    static int64_t MaxGrandParentOverlapBytes(int level, const Options* options) {
        return 10 * MaxFileSizeForLevel(level);
    }

    static int64_t ExpandedCompactionByteSizeLimit(int level, const Options* options){
        return 25 * MaxFileSizeForLevel(level);
    }
...
#pragma GCC diagnostic pop
    ...
    class Version::LevelFileNumIterator : public Iterator {
        public:
            ...
            Slice value() const {
                ...
                EncodeFixed64(value_buf_+16, (*flist_)[index_]->endOffset);
    ...
    void Version::AddSomeIterators(const ReadOptions& options, uint64_t num,
        ...
        for (size_t i = 0; i < files_[0].size(); i++) {
            if(files_[0][i]->file_size < files_[0][i]->endOffset){
                files_[0][i]->endOffset = files_[0][i]->file_size;
            }
            iters->push_back(
                    vset_->table_cache_->NewIterator(
                        options, files_[0][i]->number, files_[0][i]->file_size, files_[0][i]->endOffset));
        }
        ...
        for (unsigned level = 0; level < config::kNumLevels; level++) {
            ...
            for (uint32_t i = 0; i < num_files; ++i) {
                ...
                s = vset_->table_cache_->Get(options, f->number, f->file_size,
                        f->endOffset, ikey, &saver, SaveValue);
        ...
        if (!OverlapInLevel(0, &smallest_user_key, &largest_user_key)) {
            ...
            while (level < config::kMaxMemCompactLevel) {
                ...
                if (level + 2 < config::kNumLevels) {
                    ...
                    if (sum > MaxGrandParentOverlapBytes(level, vset_->options_)) {
                        break;
                    }
                }
                level++;
    ...
    class VersionSet::Builder {
        private:
                ...
                bool operator()(FileMetaData* f1, FileMetaData* f2) const {
                    ...
                    if (r != 0) {
                        ...
                        if(f1->number == f2->number)
                            return f1->endOffset < f2->endOffset;
            ...
            VersionSet* vset_;
            Version* base_;
            LevelState levels_[config::kNumLevels+1];
...
#pragma GCC diagnostic pop
            ...
            void Apply(VersionEdit* edit) {
                ...
                for (VersionEdit::DeletedFileSet::const_iterator iter = del.begin();
                    ...
                    levels_[level].deleted_files.insert(f);
                }

                for(int i=0,tt=config::kNumLevels;i<vset_->punched_files_.size();i++)
                    levels_[tt].deleted_files.insert(*(vset_->punched_files_[i]));
                ...
                for (size_t i = 0; i < edit->new_files_.size(); i++) {
                    ...
                    f->allowed_seeks = (f->file_size / 256);
                    if (f->allowed_seeks < 6400) f->allowed_seeks = 6400;

                    levels_[level].deleted_files.erase(*f);
            ...
            void MaybeAddFile(Version* v, unsigned level, FileMetaData* f) {
                if (levels_[level].deleted_files.count(*f) > 0)
                    {
                        // File is deleted: do nothing
                    } else {
                        std::vector<FileMetaData*>* files = &v->files_[level];
                        if (level > 0 && !files->empty() && level != config::kNumLevels)
                            {
                                // Must not overlap
...
#pragma GCC diagnostic pop
    ...
    Status VersionSet::WriteSnapshot(log::Writer* log) {
        ...
        for (unsigned level = 0; level < config::kNumLevels; level++) {
            ...
            for (size_t i = 0; i < files.size(); i++) {
                const FileMetaData* f = files[i];
                edit.AddFile(level, f->number, f->file_size, f->smallest, f->largest, f->endOffset);
            }
        }
            ...
            for (size_t i = 0; i < files.size(); i++) {
                if (icmp_.Compare(files[i]->largest, ikey) <= 0) {
                    ...
                    Iterator* iter = table_cache_->NewIterator(
                            ReadOptions(), files[i]->number, files[i]->file_size, files[i]->endOffset, &tableptr);
                    if (tableptr != NULL) {
                        result += tableptr->ApproximateOffsetOf(ikey.Encode()) 
                            - (files[i]->endOffset - files[i]->file_size);
                    }
                    delete iter;
                }
            }
    ...
    void VersionSet::PunchingFiles(std::vector<FileMetaData*> &punch_files){
        ...
        for (Version* v = dummy_versions_.next_; v != &dummy_versions_; v = v->next_) {
            for(int level = 0; level < config::kNumLevels; level++) {
                const std::vector<FileMetaData*>& files = v->files_[level];
                for(size_t i=0; i<files.size(); i++){
                    live.insert({files[i]->number, files[i]->endOffset});
                }
            }
        }

        //find punch sstables can be deleted
        for (Version* v = dummy_versions_.next_; v != &dummy_versions_; v = v->next_){
            const std::vector<FileMetaData*> &files = v->files_[config::kNumLevels];
            for(size_t i=0;i<files.size();i++){
                if(live.find({files[i]->number, files[i]->endOffset}) == live.end())
                    punch_files.push_back(files[i]);
            }
        }
    }

    void VersionSet::PunchingClear(std::vector<FileMetaData*> &punched_files){
        for(int i=0;i<punched_files.size();i++)
            punched_files_.push_back(punched_files[i]);
    }
    ...
    int VersionSet::Compare(const Slice &a, const Slice &b){
                ...
                if (c->level() + which == 0) {
                    ...
                    for (size_t i = 0; i < files.size(); i++) {
                        list[num++] = table_cache_->NewIterator(
                                options, files[i]->number, files[i]->file_size, files[i]->endOffset);
                    }
                } else {
    ...
    Compaction* VersionSet::PickCompaction(Version* v, unsigned level) {
        ...
        if (level > 0) {
            ...
            for (size_t i = 0; i < boundaries.size(); ++i) {
                uint64_t gc_cnt=1;//32;

                static const uint64_t COMPACTSIZE=(64<<20);
                for(size_t j=i+1;j<boundaries.size();j++){
                    if(LA_sizes[j+1]-LA_sizes[i+1]<COMPACTSIZE)
                        gc_cnt++;
                    else
                        break;
                }

                //for (size_t j = i+gc_cnt; j < boundaries.size(); ++j)
                size_t j = (i + gc_cnt>boundaries.size()-1?boundaries.size()-1:i+gc_cnt);
                    {
            ...
            if (trivial) {
                uint64_t total_compaction_size=0;
                for (size_t i = 0; i < LA.size(); i++) {
                    FileMetaData* f = LA[i];
                    if (compact_pointer_[level].empty() ||
                            icmp_.Compare(f->largest.Encode(), compact_pointer_[level]) > 0) {
                        c->inputs_[0].push_back(f);
                        total_compaction_size += f->file_size;
                        if(total_compaction_size >= (64<<20)){
                            break;
                        }
                        else{
                            continue;
    ...
    void VersionSet::SetupOtherInputs(Compaction* c) {
        ...
        if (level + 2 < config::kNumLevels) {
            ...
            c->input_version_->GetOverlappingInputs(level + 2, &all_start, &all_limit, &c->grandparents_);
            for (size_t i = 0; i < tmp.size(); ++i) {
...
#pragma GCC diagnostic ignored "-Wunsafe-loop-optimizations"
    ...
    void Compaction::AdjustSeek(std::vector<FileMetaData*>& victims){
        for(int i=0,j=victims.size();i<j;i++){
            FileMetaData* f = victims[i];
            f->allowed_seeks = (f->file_size / 256);
            if (f->allowed_seeks < 6400) f->allowed_seeks = 6400;
        }
    }

    void Compaction::AdjustInput(int which, std::vector<FileMetaData*>& PartitionInput){
        inputs_[which] = PartitionInput;
    }

    void Compaction::GroupingInputs(std::vector<std::vector<FileMetaData*> >& BaseLevel,
            std::vector<std::vector<FileMetaData*> >& TargetLevel,
            std::vector<FileMetaData*>& Move,
            std::vector<Slice>& SplitKey){
        const InternalKeyComparator* icmp = &input_version_->vset_->icmp_;
        const VersionSet* vset = input_version_->vset_;
        int BaseIdx=0, TargetIdx=0;
        int BaseLimit = inputs_[0].size(), TargetLimit = inputs_[1].size();
        std::vector<FileMetaData*> base, target, grandpa, tmpbase, tmp;
        std::vector<std::pair<int, Slice> > valid;
        std::vector<Slice> splitA, splitB;
        for(int i=0;i<BaseLimit;i++){
            bool candidate=false;
            Slice small = inputs_[0][i]->smallest.Encode();
            Slice large = inputs_[0][i]->largest.Encode();
            // overlap to next level
            if (input_version_->OverlapInLevel(level_ + 1, &small, &large)) {
                candidate=true;
            }
            else if(level_ + 2 < config::kNumLevels){
                input_version_->GetOverlappingInputs(level_ + 2, &inputs_[0][i]->smallest, &inputs_[0][i]->largest, &grandpa);
                if(TotalFileSize(grandpa) > MaxGrandParentOverlapBytes(level_, vset->options_))
                    candidate=true;
            }

            // just move
            if(!candidate){
                splitA.push_back(inputs_[0][i]->smallest.Encode());
                Move.push_back(inputs_[0][i]);
            }
            // candidates for grouping
            else{
                base.push_back(inputs_[0][i]);
            }
        }

        if(base.size()>0){
            input_version_->GetOverlappingInputs(level_ + 1, &base[0]->smallest, &base[base.size()-1]->largest, &tmp);

            TargetLimit = tmp.size();
            for(int i=0;i<TargetLimit;i++){
                Slice small = tmp[i]->smallest.Encode();
                Slice large = tmp[i]->largest.Encode();
                if(input_version_->OverlapInLevel(level_, &small, &large))
                    target.push_back(tmp[i]);
                else
                    splitB.push_back(tmp[i]->smallest.Encode());
            }

            int i=0,j=0,r=0;
            while(i<splitA.size() && j<splitB.size()){
                r = icmp->Compare(splitA[i],splitB[j]);
                if(r<0)
                    SplitKey.push_back(splitA[i++]);
                else
                    SplitKey.push_back(splitB[j++]);
            }
            ...
            BaseLevel.push_back(base);
            TargetLevel.push_back(target);
        }
...
#pragma GCC diagnostic pop
    ...
    bool Compaction::ShouldStopBefore(const Slice& internal_key) {
        ...
        if (overlapped_bytes_ > MaxGrandParentOverlapBytes(level_, vset->options_)) {
    ...
    bool Compaction::IsTrivialMove() const {
        ...
        return (num_input_files(0) == 1 && num_input_files(1) == 0 && 
                TotalFileSize(grandparents_) <= MaxGrandParentOverlapBytes(level_, vset->options_));
    }
    ...
    void Compaction::AddInputDeletions(VersionEdit* ed) {
        for (int which = 0; which < 2; which++) {
            for (size_t i = 0; i < inputs_[which].size(); i++) {
                ...
                ed->DeleteFile(level_ + which,f);
                ed->AddFile(config::kNumLevels, f->number, f->file_size, 
                        f->smallest, f->largest, f->endOffset);
            }
        }
    }
