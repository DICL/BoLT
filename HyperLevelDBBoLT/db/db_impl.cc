...
const unsigned kStraightReads = 0;
...
Status DBImpl::DoCompactionWork(CompactionState* compact) {
  ...
  std::vector<FileMetaData*> original_inputs[2];
  ...
  if(group_flag){
      ...
      if(Move.size()!=0){
          ...
          for(int i=0;i<Move.size();i++){
              ...
              compact->compaction->edit()->AddFile(level_+1, f->number, f->file_size, \
                      f->smallest, f->largest, f->endOffset);
      ...
      if(BaseLevel.size()==0){
          ...
          Log(options_.info_log, "No BaseLevel on Compaction %lu@%d + %lu@%d files",
                  compact->compaction->num_input_files(0),
                  compact->compaction->level(),
                  compact->compaction->num_input_files(1),
                  compact->compaction->level()+1);

          original_inputs[0] = compact->compaction->inputs(0);
          original_inputs[1] = compact->compaction->inputs(1);
          ...
          if(status.ok()){
              compact->compaction->AdjustSeek(original_inputs[0]);
              compact->compaction->AdjustSeek(original_inputs[1]);
