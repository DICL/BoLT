...
static void DeleteFileTable(void* f, void* t){
    RandomAccessFile *file = reinterpret_cast<RandomAccessFile*>(f);
    Table *table = reinterpret_cast<Table*>(t);
    delete file;
    delete table;
}
...
Status TableCache::FindTable(uint64_t file_number, uint64_t file_size,
        uint64_t endOffset, Cache::Handle** handle) {
    Status s;
    ...
    Cache::Handle* handle2 = file_cache_->Lookup(fkey);
    ...
    if(handle2 == nullptr){
        ...
        handle2 = file_cache_->Insert(fkey,file,file_size,&DeleteFile);
    }
    else{
...
void TableCache::Evict(uint64_t file_number, uint64_t endOffset) {
  char buf[sizeof(endOffset)+sizeof(file_number)];
  EncodeFixed64(buf, endOffset);
  EncodeFixed64(buf+sizeof(endOffset), file_number);
