...
// mmap() based random-access
class PosixMmapReadableFileWithSize: public RandomAccessFile {
 private:
  PosixMmapReadableFileWithSize(const PosixMmapReadableFileWithSize&);
  PosixMmapReadableFileWithSize& operator = (const PosixMmapReadableFileWithSize&);
  std::string filename_;
  void* mmapped_region_;
  size_t length_;
  const size_t start_;
 ...
 public:
  // base[0,length-1] contains the mmapped contents of the file.
  PosixMmapReadableFileWithSize(const std::string& fname, void* base, size_t length, size_t start,
                        Limiter* limiter
                        )
      : filename_(fname), mmapped_region_(base), length_(length), start_(start),
        limiter_(limiter) {
  }

  virtual ~PosixMmapReadableFileWithSize() {
    munmap(mmapped_region_, length_);
    limiter_->Release();
  }
...
class PosixEnv : public Env {
 public:
  ...
  virtual Status NewRandomAccessFileWithSize(const std::string& fname,
                                     RandomAccessFile** result, uint64_t start, uint64_t endOffset) {
    *result = NULL;
    Status s;
    ...
    if (fd < 0) {
      ...
      if (s.ok()) {
        ...
        uint64_t file_size = endOffset-start;
        void* base = mmap(NULL, file_size, PROT_READ, MAP_SHARED, fd, start);
        if (base != MAP_FAILED) {

          *result = new PosixMmapReadableFileWithSize(fname, base, file_size, start, &mmap_limit_);
        } else {
          s = IOError(fname, errno);
        }
      }
